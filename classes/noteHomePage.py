...
class noteHomePage(tk.Frame):
    def __init__(self, parent):
        ...
        noteHomePage.notes = tk.Listbox(self)
        noteHomePage.notes.bind("<Double-Button-1>", self.on_double_click)

        for i, k in enumerate(Note.notehash.keys()):
            noteHomePage.notes.insert(i, k)

        self.addNoteLabel = tk.Label(
            self, text="Notes", font=("Arial", 16, "bold"))

        self.buttonFrame = tk.Frame(self)

        self.buttonDelete = tk.Button(
            self.buttonFrame, text='delete', command=self.remove_item)
        self.buttonAdd = tk.Button(
            self.buttonFrame, text='add', command=self.addNote)
        self.buttonSearch = tk.Button(
            self.buttonFrame, text='search', command=self.search_item)
        ...
        noteHomePage.notes.grid(row=1, column=0, padx=10, pady=10)

        # self.update_notes_list()
    @classmethod
    def update_notes_list(cls, title):

        noteHomePage.notes.insert(tk.END, title)
